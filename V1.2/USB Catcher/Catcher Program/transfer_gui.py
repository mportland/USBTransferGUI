...
def load_gui(self):
    ...
    self.name_label = tk.Label(self.master, text="Enter the name of file").grid(row=1, column=0, sticky=N)
    self.txt_filename = tk.Entry(self.master,text='Filepath Here')
 ...
 # Create button to begin file creation
    self.btn_scncpy = tk.Button(self.master, width=12, height=2, text='Begin Transfer',
                                command=lambda: transfer_func.begin_transfer(self))
    ...
    self.btn_reset = tk.Button(self.master, width=12, height=2, text='Reset',
                               command=lambda: transfer_func.reset(self))
